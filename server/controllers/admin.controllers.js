...
const adminLogin = TryCatch(async (req, res, next) => {
  ...
  // If the keys do not match, return an error (invalid admin key).
  if (!isMatched) {
    return next(new ErrorHandler("Invalid Admin key", 401));
  }

  // If the secretKey is correct, create a JWT token with admin information.
  // The payload could be more detailed, like user details, but here we use a simple admin flag.
  const token = jwt.sign({ admin: true }, process.env.JWT_SECRET);

  

  // Send the token back to the client within a secure cookie, and return a success response.
  return res
    .status(200) // Respond with a status of 200 (OK).
    .cookie("chatapp-admin-token", token, {...cookieOptions, maxAge: 100*60*15,}) // Set the cookie with the JWT token.
    .json({
      success: true, // Indicate that the authentication was successful.
      message: "Authenticated successfully, Welcome!", // Provide a success message to the user.
    });
});
...
const adminLogout = TryCatch(async (req, res, next) => {
    ...
    return res
      .status(200) // Respond with a status of 200 (OK).
      .cookie("chatapp-admin-token", "", {...cookieOptions, maxAge: 0,}) // Set the cookie with the JWT token.
      .json({
        success: true, // Indicate that the authentication was successful.
...
const allMessages = TryCatch(async (req, res) => {
  ...
  const tranformedMessages = messages.map(
